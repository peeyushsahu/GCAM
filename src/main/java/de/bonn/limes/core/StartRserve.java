...
public class StartRserve {
	...
	public static boolean isRserveRunning() {
        ...
        //This method will analyze the RScript
        
           void RscriptAnalyze(String path, Integer threshold){
           checkLocalRserve();
           try {
                    RConnection c=new RConnection();
                    ...
                    REXP cellCount = c.eval("palindrome("+path+","+threshold+")");
                    System.out.println("Analysis is done and heatmap is plotted:   "+cellCount.asInteger()); // prints 1 => true
                    System.out.println("RServer is shutting down....");
                    c.shutdown();
            } catch (Exception x) {};
